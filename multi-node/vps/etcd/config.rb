...
ETCD_INDX = ENV.fetch('ETCD_PUBLIC_IPS').split(',').index(ENV.fetch('COREOS_PUBLIC_IPV4'))
...
data['coreos']['etcd2']['initial-cluster'] = initial_cluster(ENV.fetch('ETCD_PRIVATE_IPS'))
